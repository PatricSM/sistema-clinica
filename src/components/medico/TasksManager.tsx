...
interface TaskWithPatient extends TarefaPaciente {
  patient_name?: string;
}
...
export default function TasksManager({ professionalId }: TasksManagerProps) {
  ...
  const fetchTasks = async () => {
    ...
    try {
      ...
      // Adicionar nomes dos pacientes
      const tasksWithPatients = tasksData?.map(task => {
        const patient = patients.find(p => p.id === task.patient_id);
        return {
          ...task,
          patient_name: patient?.user?.full_name || `Paciente ${task.patient_id}`
        };
      }) || [];

      setTasks(tasksWithPatients);
    } catch (error) {
  ...
  const fetchPatients = async () => {
    try {
      ...
      // (usando o professionalId que já é passado como prop)
      
      // Buscar pacientes através dos appointments com join
      ...
      // Remover pacientes duplicados e formatar dados
      const pacientesUnicos = new Map()
      
      appointmentsData?.forEach((appointment: any) => {
        const patientId = appointment.patient.id
        if (!pacientesUnicos.has(patientId) && appointment.patient.status === 'active') {
          pacientesUnicos.set(patientId, {
            id: patientId,
            user: { 
              full_name: appointment.patient.users.full_name 
            }
          })
        }
      })

      const patientsArray = Array.from(pacientesUnicos.values())
      setPatients(patientsArray)
      
      // console.log('Pacientes carregados:', patientsArray) // Debug removido
    } catch (error) {
      console.error('Erro ao buscar pacientes:', error)
      // Em caso de erro, não usar dados hardcoded, deixar vazio
      setPatients([])
    }
  };
